...
# -*- coding: utf-8 -*-
"""
從 PPOCRLabel 標註數據準備 PaddleOCR 訓練數據（正確的透視變換版本）
使用透視變換裁剪傾斜的文字區域，保持正確的寬高比
"""
...
def crop_text_region_correct(image, points):
    """
    使用透視變換正確裁剪文字區域
    關鍵：計算正確的寬高，保持原始比例
    """
...
def prepare_recognition_data(data, base_dir, output_dir, train_ratio=0.9):
    ...
    for item in tqdm(data, desc="處理圖片"):
        ...
        for ann in item['annotations']:
            ...
            # 裁剪文字區域
            try:
                cropped = crop_text_region_correct(image, points)
...
def main():
    ...
    print("PaddleOCR 數據準備工具 (正確的透視變換版)")
    print("=" * 50)
